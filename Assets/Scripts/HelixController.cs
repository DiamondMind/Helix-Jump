...
public class HelixController : MonoBehaviour {
    ...
    private List<BallController> ballControllers = new List<BallController>();
    ...
    void Awake () {
        ...
        LoadStage(0);
        ballControllers = GameManager.ballControllers;
    }
    ...
    void Update () {
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            float delta = (lastTapPos.x - curTapPos.x) * rotateSensitivity; // Apply sensitivity.
            lastTapPos = curTapPos;
    ...
    {
        // Check if the stageNumber is within the valid range
        if (stageNumber < 0 || stageNumber >= allStages.Count)
        {
            //Debug.LogError("Invalid stageNumber: " + stageNumber + " (HelixController).");
            //return;
            stageNumber = 0;
        }
        ...
        GetComponent<MeshRenderer>().material.color = allStages[stageNumber].helixColor;
